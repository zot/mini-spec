...
## Does
- missingTrajectory(repoRoot): the typed refusal every queue verb makes first when any of
  the three files is absent, naming them (R332)
- CreateTrajectory(repoRoot): write each missing trajectory file with its lifecycle preamble,
  never an existing one; returns what it wrote (R333)
- CreateCarve(repoRoot, name): scaffold `carves/<name>.md` with one open part, refusing an
  existing file or a path-like name (R334)
