...
| `minispec validate trajectory` | [validate.md](validate.md) | Check the trajectory layer — the queue files at the repository root against the carves that point at them — in both directions, plus the ledger, numbering, marking-agreement and current-file shape checks. Repository-scoped, never a design root; `make validate` runs both. Since 2026-09-15 also the reference discipline: every link in every tracked markdown file classified as `query links` does; `ignored`, `missing` and `outside` fail the phase, `untracked` is a note, and outside a git tree the note says the links went unclassified; a done entry whose `Part` pointer names a missing document, and the ledgers' unresolvable links, are notes. |
...
| `query links [--all] [file...]` | [queries.md](queries.md), [links-schema.md](links-schema.md) | Every markdown link in the named documents (default: every tracked markdown file), resolved relative to the citing file and classified against git: `tracked`, `untracked` (warning), `ignored` (error), `missing` (error), `outside` (error), `external`, `local`. Links inside code groups are never read. Lines carrying a decision are listed, `--all` lists every link, the closing count states every class; exit 1 on any error; refuses outside a git tree. Needs no design root. |
...
| `pending commit-message [--amend] [--out <file>]` | [queue-items.md](queue-items.md) | Composes the commit message for every done entry no commit on `HEAD`'s history names (`#N` bounded by a non-digit): subject `#N, #M: titles`, body `Items #N, #M.` then each entry with its done-file body. `--amend` returns `HEAD`'s message unchanged with the new items appended, refusing when `HEAD` is on a remote. Never commits. Needs no design root. |
| `pending finish <N> [--body <text>\|--body-file <path>] [--discharged <text>\|--discharged-file <path>] (--resolve\|--no-resolve)` | [queue-items.md](queue-items.md) | Check off each discharged part in its carve — box, strikethrough, appended `LANDED` record — then reset `## Active`, then move the entry to the done file with its header (`#N / <discharged>`) and the body when given. A gap-sourced item must say `--resolve` or `--no-resolve`. The `written` line names each file's git kind — tracked and uncommitted, ignored, untracked — so the carve flip reads as the write that still needs a commit. |
...
| `update finished-carve <carve>` | [updates.md](updates.md) | Moves a carve directly in `carves/` or `.carves/` to its `done/`, rewriting every link and every `doc.md#key` pointer that resolves to it, in every tracked markdown file and every sited document — exactly what `query refs --to` lists and every resolving link inside it, from where they resolve now; refuses before any write on an open part, no status block, a present target or a read-back failure; a link that does not resolve is left and reported. Plain rename, nothing staged. Needs no design root. |
| `update repair-links [file...]` | [updates.md](updates.md), [links-schema.md](links-schema.md) | Rewrites links a carve's move broke, both directions: a `missing` link that exactly one sibling relocation between `carves/` and `carves/done/` resolves gets its destination bytes rewritten through the `Markdown` reader; none or two resolving is reported and left. Default population: every tracked markdown file plus every sited document. Reports every link considered with counts; exit 1 when any was left. Needs no design root. |
