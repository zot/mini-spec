...
| [queue-items.md](queue-items.md) `pending commit-message` | the message that names every item a commit lands, composed from the done entries no commit names yet; `--amend` appends after `HEAD`'s message and refuses on a remote |
...
| [queries.md](queries.md) `query refs` | the inventory of links and pointers, `--to` inverted; the move is specified against it |
...
| [links-schema.md](links-schema.md) | any markdown document's links — the inline form, outside code groups, with line and offset; the round trip over the real corpus |
